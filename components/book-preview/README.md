...
- The premier engine is the flagship book: four remembered views — the full curl stage (corner drag, tap zones, edge stacks, page-turn sound), a single-page view, a two-page spread, and a continuous scroll — plus page-level search, zoom, thumbnails, the PDF outline as contents, and continuous read-aloud that turns its own pages and skips silent leaves. Page data and PDFs alike; the flat views rasterize only a window around the reading position, so documents past `CURL_MAX_PAGES` still read there while the flip book bows out.
