...
export function BookPreviewViewport({ children }: { children?: ReactNode }) {
  ...
  return (
    <div
      className="relative isolate min-h-[min(22rem,68svh)] w-full min-w-0 overflow-hidden rounded-xl border bg-card sm:min-h-[min(32rem,74svh)] lg:min-h-[min(40rem,80svh)] xl:min-h-[min(48rem,84svh)]"
      data-book-preview-surface
      data-book-preview-viewport
    >
      {!blocking && state.totalPages > 1 ? (
        <div
          className="absolute inset-x-0 top-0 z-20 h-0.5 overflow-hidden rounded-t-xl bg-foreground/10"
          aria-hidden
        ...
        >
          ...
          <div
            className="h-full w-full origin-left bg-foreground/50 transition-transform duration-300 ease-out motion-reduce:transition-none"
            style={{
      ...
      {blocking ? (
        <div className="absolute inset-0 z-10">
          <BookPreviewStatus
            ...
            onFallback={
              pageEngine && state.mode !== "page" ? () => setMode("page") : undefined
            }
          />
        </div>
      ) : null}
      <div
        ...
        className={cn("absolute inset-0", blocking && "pointer-events-none opacity-0")}
